  ...
  return (
    ...
    >
      <div
        ...
        className={`tilt-card relative h-full p-8 md:p-10 rounded-3xl bg-card border ${
          item.current ? "border-primary/60 shadow-glow" : "border-border"
        }`}
      >
        {item.current && (
          <span className="absolute -top-3 left-8 px-3 py-1 text-[10px] uppercase tracking-widest rounded-full bg-primary text-primary-foreground">
            Current
          </span>
        )}
        <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">{item.period}</p>
        <h3 className="font-serif text-3xl mt-3 leading-tight">{item.org}</h3>
        ...
        <div className="mt-3 font-serif text-5xl text-primary/20">0{index + 1}</div>
      </div>
    </motion.div>
  );
};
    ...
    <section id="experience" className="py-28 md:py-36 bg-secondary/30">
      <div className="container">
        <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
          <div>
            <p className="text-xs uppercase tracking-[0.4em] text-primary mb-4">— experience</p>
            <h2 className="font-serif text-4xl md:text-6xl">Where I've <span className="italic">grown.</span></h2>
          </div>
